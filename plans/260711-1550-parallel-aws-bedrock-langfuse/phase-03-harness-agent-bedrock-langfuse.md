...
chat with a persona picker. Developed 100% against the Phase 2 mock.
...
Streamlit chat ── run_agent(question, persona)          harness/agent/
                     ...
                     └─ tool dispatch → subprocess: $GOVERNED_CLI_CMD --token $TOKEN graph|query "..."
                          @observe span per tool call; JSON envelope passed back as toolResult
```
...
4. `system_prompt.py`: governed-analyst prompt (KG-first, cite caveats/notices, VND/tỷ
   formatting).
5. `app.py`: chat + persona dropdown (reads `contracts/personas.json`), history/session
   in `st.session_state`, per-turn `langfuse.flush()`, and a shared-passphrase gate
   (env `DEMO_PASSPHRASE`) so a public URL can't burn Bedrock quota; new chat = new
   Langfuse session id.
6. Manual eval: run 3–4 TRAPS.md questions against the mock; verify nested traces, token
