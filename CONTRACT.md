# CONTRACT.md — the governed CLI interface (v0.2)
...
Everything in this file is **normative**: the harness may rely on it, and nothing else. How the services behind the CLI are built (semantic-layer YAML, vocabulary files, the KG compile step, governance internals) is intentionally *not* specified here — see the service READMEs; none of it is stable interface.
...
|---|---|---|---|
| `tok-exec-mai` | Mai, CEO | `executive` | Full metric/table access. `applied_permissions` still lists tokenized customer ids. Happy path with full provenance |
| `tok-rm-south-duc` | Đức, Regional Manager South | `regional_manager` | Store-grained queries return **South-only** results; `applied_permissions.row_filters` discloses `dim_store.region = 'South'`. Customer-PII-backed dimensions are denied |
| `tok-mkt-lan` | Lan, Marketing Ops | `marketing_ops` | `fact_inventory` → `ACCESS_DENIED_TABLE`; margin/cost metrics → `ACCESS_DENIED_METRIC` with reason `"uses masked column cost_amount"`; customer birth years appear as 5-year bands; customer ids tokenized |
...
`tn whoami --token T` returns the resolved persona and effective permissions:
`result`: `{user, role, readable_tables[], row_filters[], masked_columns[], tokenized_columns[], denied_metrics[]}` — all display strings, for narration, not for parsing.
...
- `applied_permissions` is **always present and truthful** — the agent narrates from it ("your view is scoped to region South"). Row-level security is disclosed, never silent. Entries are display strings, not parseable SQL.
...
- **Freshness**: tables can be differently stale by design. `metadata.freshness` gives max available date per touched table; `as_of` = the minimum of those. A `STALE_DATA` warning fires when they diverge.
- **Numbers**: monetary values are integer **VND** (column metadata carries `"unit": "VND"`); ratios are **0–1 fractions** with `"unit": "ratio"` (percent formatting is the harness's job). All numeric JSON values are guaranteed to fit exactly in an IEEE-754 double (|n| ≤ 2^53−1); the CLI errors rather than silently rounding if an aggregate would exceed it (full-scale totals are ~10^14, well inside).
...
`result`: `{key, name, description, type (sum|count|ratio|derived), unit, formula (human-readable), tables[], dimensions[]: {key, name, type, description}, constraints[]: {key, statement}, concept: {key, name}, access}`. Exact key only. This and the KG are the two ways to learn descriptions and types; they mirror the same keys.
