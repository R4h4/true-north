...
from tests.conftest import FakeNode, FakePath, FakeRecord, FakeRel
...
def test_metric_node_shape_matches_fixture(access_index):
    ser = Serializer(access_index, "data_analyst")
    node = FakeNode("Metric", {
...
def test_denied_metric_carries_reason(access_index):
...
def test_dimension_node_shape_no_type(access_index):
    ser = Serializer(access_index, "data_analyst")
    node = FakeNode("Dimension", {
...
def test_concept_and_constraint_have_no_access(access_index):
    ser = Serializer(access_index, "marketing_ops")
    concept = FakeNode("Concept", {"key": "customer_retention", "name": "Customer Retention",
                                   "definition": "…", "aliases": ["retention"]})
    constraint = FakeNode("Constraint", {"key": "b2b_value_skew", "statement": "…", "severity": "warning"})
    assert "_access" not in ser.value(concept)
    assert "_access" not in ser.value(constraint)


def test_detail_record_shape_matches_fixture(access_index):
    """A full metric-detail record (m + collect(dims/caveats/tables)) matches shape."""
    ser = Serializer(access_index, "data_analyst")
    m = FakeNode("Metric", {"key": "net_revenue", "name": "N", "description": "d",
                            "type": "derived", "unit": "VND", "formula": "f", "version": "0.1"})
    d = FakeNode("Dimension", {"key": "channel", "name": "c", "description": "d",
                              "canonical_values": ["app"]})
    k = FakeNode("Constraint", {"key": "b2b_value_skew", "statement": "s", "severity": "warning"})
    t = FakeNode("Table", {"key": "fact_sales_lines", "description": "d",
                           "grain": "sales line", "freshness_note": "refreshed daily"})
    record = FakeRecord({"m": m, "dims": [d], "caveats": [k], "tables": [t]})
...
def test_relationship_serialization(access_index):
    ser = Serializer(access_index, "data_analyst")
    a = FakeNode("Concept", {"key": "repeat_purchase_retention", "name": "n"})
    b = FakeNode("Metric", {"key": "repeat_purchase_rate_90d", "name": "n", "description": "d",
                            "type": "ratio", "unit": "ratio", "formula": "f", "version": "0.1"})
    rel = FakeRel("MEASURED_BY", a, b)
    out = ser.value(rel)
...
def test_path_serialization(access_index):
    ser = Serializer(access_index, "data_analyst")
    a = FakeNode("Concept", {"key": "basket_by_value", "name": "n"})
    b = FakeNode("Metric", {"key": "basket_value_avg", "name": "n", "description": "d",
...
def test_scalar_nan_and_dates(access_index):
